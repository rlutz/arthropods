...
if len(sys.argv) != 3 or sys.argv[1] not in variant_map.keys():
    print >>sys.stderr, "Usage: %s %s <pin_count>" % (
        sys.argv[0], '|'.join(variant_map.keys()))
    sys.exit(1)
...
if variant == SIP:
    ...
    if pin_count % 2 != 0:
        print >>sys.stderr, "DIP pin count must be even"
        sys.exit(1)
    rows = 2
